...
// requireAuth() redirects unauthenticated visitors, but the redirect
// doesn't halt this script — so every DOMContentLoaded handler below
// checks `currentUser` before touching it, instead of assuming it's set.
const isAuthed = requireAuth();
const currentUser = isAuthed ? getCurrentUser() : null;
...
function loadTasks(){
  // readArray() falls back to [] for missing/corrupted data, then we
  // drop any individual records that are missing the fields the
  // renderer needs, so old/invalid localStorage can't crash the app.
  tasks = readArray(tasksKey()).filter(t => t && typeof t.id === 'string' && typeof t.title === 'string');
}
...
document.addEventListener('DOMContentLoaded', () => {
  if(!currentUser) return; // requireAuth() is already redirecting to login
...
function renderListView(view, container){
  ...
  // Empty-state copy is chosen in priority order so it always tells
  // the person the actual reason the list is empty:
  //  1. they have no tasks at all yet
  //  2. a search term is active and nothing matched it
  //  3. a filter is active (My Tasks) and nothing matched it
  //  4. the view itself is naturally empty (e.g. nothing due today)
  let title, sub;
  if(tasks.length === 0){
    title = 'No tasks yet';
    sub = 'Create your first task and start getting things done.';
  } else if(isSearching){
    title = 'No matching tasks found.';
    sub = `Nothing matches "${searchQuery}". Try a different search term.`;
  } else if(view === 'tasks' && activeFilter !== 'all'){
    title = 'No tasks match this filter';
    sub = 'Try a different filter, or choose "All" to see everything.';
  } else {
    const emptyMessages = {
      tasks: ['No tasks yet', 'Create your first task and start getting things done.'],
    ...
    [title, sub] = emptyMessages[view] || ['Nothing here', ''];
  }
...
function bindModal(){
  ...
  form.addEventListener('submit', (e) => {
    ...
    if(!wasEditing){
      // A brand-new task must be visible immediately. If the person
      // added it from the Dashboard (which only previews *today's*
      // tasks) or while a filter/search was narrowing My Tasks, jump
      // to My Tasks and clear anything that could hide the new task.
      activeFilter = 'all';
      syncFilterChipsUI();
      searchQuery = '';
...
/* Escape closes whichever modal is currently open (task modal takes
   priority if, somehow, both were open at once). One listener for
   both dialogs instead of a separate handler per modal. */
document.addEventListener('keydown', (e) => {
  if(e.key !== 'Escape') return;
  const taskModal = document.getElementById('task-modal');
  const confirmModal = document.getElementById('confirm-modal');
  if(taskModal && taskModal.classList.contains('open')) closeModal();
  else if(confirmModal && confirmModal.classList.contains('open')) closeConfirmModal();
});
